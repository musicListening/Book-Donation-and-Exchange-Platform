...
export default function StaffProfile() {
  ...
  useEffect(() => {
    const stored = JSON.parse(localStorage.getItem('user') || localStorage.getItem('ss_current_user') || '{}');
    if (stored) {
      setUser(stored);
      setName(stored.name || '');
  ...
  const initials = name ? name.split(' ').map(w => w[0]).join('').toUpperCase().slice(0, 2) : 'ST';
  ...
  return (
    <StaffLayout title="Profile">
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '32px 24px' }}>
        {message && (
          <div style={{
            padding: '12px 20px', borderRadius: 8, marginBottom: 24, fontWeight: 600, fontSize: 14,
        ...
        <div style={{ background: 'white', borderRadius: 16, boxShadow: '0 2px 12px rgba(0,0,0,0.06)', overflow: 'hidden' }}>
          <div style={{ height: 120, background: 'linear-gradient(135deg, #1E4D4B 0%, #2A9D8F 100%)' }} />
          <div style={{ padding: '0 40px 40px', marginTop: -48 }}>
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: 24, marginBottom: 32 }}>
              <div style={{
                ...
                background: '#1E4D4B', overflow: 'hidden', boxShadow: '0 4px 12px rgba(0,0,0,0.15)',
                ...
                <label style={{
                  ...
                  fontSize: 11, fontWeight: 600, padding: '4px 0', cursor: 'pointer',
                }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              <div>
                ...
                <input type="text" value="Operations Staff" readOnly
                  style={{ width: '100%', padding: '12px 16px', borderRadius: 8, border: '1px solid #DEE2E6', fontSize: 15, background: '#F8F9FA', color: '#6C757D', cursor: 'not-allowed' }} />
              </div>
            </div>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12, marginTop: 32, borderTop: '1px solid #F1F3F5', paddingTop: 24 }}>
              ...
              <button onClick={handleSave} disabled={saving}
                style={{ padding: '10px 24px', borderRadius: 8, border: 'none', background: '#1E4D4B', color: 'white', fontWeight: 600, fontSize: 14, cursor: saving ? 'not-allowed' : 'pointer', opacity: saving ? 0.7 : 1, display: 'flex', alignItems: 'center', gap: 8 }}>
