...
export default function Navbar({
  ...
  const closeAuthModal = () => setAuthModal({ isOpen: false, mode: 'login', destination: null });
  const openAuthModal = (mode = 'login', destination = null) => setAuthModal({ isOpen: true, mode, destination });
  const getRedirectPath = (role) => {
    switch (role) {
      case 'PLATFORM_ADMIN': return '/admin/dashboard';
      case 'OPERATIONS_STAFF': return '/staff/dashboard';
      case 'DELIVERY_PERSONNEL': return '/delivery/DeliveryPersonPage';
      case 'COMMUNITY_ADMIN': return '/community-admin/dashboard';
      default: return '/user-dashboard';
    }
  };

  const handleProtectedAction = (destination, mode = 'login') => {
      ...
      <header className={`ss-navbar ${scrolled ? 'ss-navbar--scrolled' : ''} ${mobileOpen ? 'ss-navbar--open' : ''}`}>
        <div className="ss-navbar__inner">
              ...
              {links.map((link) =>
                link.hash ? (
                  ...
                  <button
                    key={link.label}
                    className="ss-navbar__link ss-navbar__link--btn"
                    onClick={() => handleProtectedAction(link.to)}
                  >
                    {link.label}
                  </button>
                ) : (
          ...
          <div className="ss-navbar__actions">
            {variant === 'public' && (
              <>
                <button onClick={() => openAuthModal('signup', null)} className="ss-navbar__btn ss-navbar__btn--ghost">Sign Up</button>
                <button onClick={() => openAuthModal('login', null)} className="ss-navbar__btn ss-navbar__btn--primary">Log In</button>
              </>
            )}
            ...
            {variant === 'community' && (
              <button onClick={() => openAuthModal('login', null)} className="ss-navbar__btn ss-navbar__btn--primary">
                Sign In
              </button>
            )}
        ...
        <div className={`ss-navbar__mobile ${mobileOpen ? 'ss-navbar__mobile--open' : ''}`}>
            ...
            links.map((link) =>
              link.hash ? (
                ...
                <button
                  key={link.label}
                  className="ss-navbar__mobile-link"
                  onClick={() => { setMobileOpen(false); handleProtectedAction(link.to); }}
                >
                  {link.label}
                </button>
              ) : (
          ...
          <div className="ss-navbar__mobile-actions">
            {variant === 'public' && (
              <>
                <button onClick={() => { setMobileOpen(false); openAuthModal('signup', null); }} className="ss-navbar__btn ss-navbar__btn--ghost">Sign Up</button>
                <button onClick={() => { setMobileOpen(false); openAuthModal('login', null); }} className="ss-navbar__btn ss-navbar__btn--primary">Log In</button>
              </>
            )}
            {variant === 'user' && user && (
