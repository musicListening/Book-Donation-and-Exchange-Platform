...
const Home = () => {
  ...
  const [authModal, setAuthModal] = useState({ isOpen: false, mode: 'login', destination: null });
  ...
  const getRedirectPath = useCallback((role) => {
    switch (role) {
      ...
      case 'DELIVERY_PERSONNEL':
        return '/delivery/DeliveryPersonPage';
  ...
  const closeAuthModal = useCallback(() => {
    setAuthModal({ isOpen: false, mode: 'login', destination: null });
  }, []);

  const openAuthModal = useCallback((mode = 'login', destination = null) => {
    setAuthModal({ isOpen: true, mode, destination });
  }, []);

  const handleProtectedAction = useCallback((destination, mode = 'login') => {
    const token = localStorage.getItem('token');
    if (token) {
      navigate(destination);
      return;
    }
    openAuthModal(mode, destination);
    ...
    <div style={styles.body}>
        ...
        <div style={styles.hero}>
          <div style={styles.heroContent}>
            ...
            <div className="hero-enter hero-enter--delay-4" style={styles.heroButtons}>
              <button type="button" onClick={() => handleProtectedAction('/donate')} className="hero-btn-primary" style={{ ...styles.btn, backgroundColor: '#ffffff', color: '#1E4D4B', border: 'none', boxShadow: '0 4px 14px rgba(255, 255, 255, 0.15)' }}>
                <i className="fa-solid fa-hand-holding-heart"></i> Start Donating
              </button>
              <button type="button" onClick={() => handleProtectedAction('/marketplace')} className="hero-btn-secondary" style={{ ...styles.btn, backgroundColor: 'transparent', color: '#ffffff', border: '2px solid rgba(255, 255, 255, 0.4)' }}>
      ...
      <section id="marketplace" style={styles.categories}>
        ...
        <div className="reveal-stagger" style={styles.categoryGrid}>
          <div className="card-hover-lift" style={styles.categoryCard} onClick={() => handleProtectedAction('/marketplace')}>
            ...
            <div style={styles.bundleDetails}>
              ...
              <div style={styles.bundlePriceRow}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button type="button" onClick={() => handleProtectedAction('/cart')} style={{ ...styles.btn, ...styles.btnPrimary, ...styles.btnSm }}>Add to Cart</button>
                  <button type="button" onClick={() => handleProtectedAction('/marketplace')} style={{ ...styles.btn, ...styles.btnSecondary, ...styles.btnSm }}>Wishlist</button>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="card-hover-lift" style={styles.bundleCard}>
            ...
            <div style={styles.bundleDetails}>
              ...
              <div style={styles.bundlePriceRow}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button type="button" onClick={() => handleProtectedAction('/cart')} style={{ ...styles.btn, ...styles.btnPrimary, ...styles.btnSm }}>Add to Cart</button>
                  <button type="button" onClick={() => handleProtectedAction('/wishlist')} style={{ ...styles.btn, ...styles.btnSecondary, ...styles.btnSm }}>Wishlist</button>
                </div>
              </div>
            </div>
          </div>
        ...
        <div style={styles.footerGrid}>
          ...
          <div>
            ...
            <ul style={styles.footerLinks}>
              <li style={{ marginBottom: 8 }}><button type="button" onClick={() => handleProtectedAction('/donate')} style={{ color: 'rgba(255,255,255,0.7)', textDecoration: 'none', fontSize: 14, background: 'none', border: 'none', padding: 0, cursor: 'pointer' }}>Donate Books</button></li>
              <li style={{ marginBottom: 8 }}><button type="button" onClick={() => handleProtectedAction('/marketplace')} style={{ color: 'rgba(255,255,255,0.7)', textDecoration: 'none', fontSize: 14, background: 'none', border: 'none', padding: 0, cursor: 'pointer' }}>Marketplace</button></li>
      ...
      <AuthModal
        ...
        onSuccess={(role) => {
          const destination = authModal.destination || getRedirectPath(role);
          closeAuthModal();
          navigate(destination);
        }}
      />
