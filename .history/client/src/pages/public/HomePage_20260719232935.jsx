...
const Home = () => {
  ...
  // Updated openAuthModal with better redirect handling
  const openAuthModal = useCallback((mode = 'login', redirect = null) => {
  ...
  // Updated handleProtectedAction
  const handleProtectedAction = useCallback((path) => {
    const token = localStorage.getItem('token');
    if (token) {
      navigate(path);
    } else {
  ...
  const handleLoginSuccess = useCallback((token) => {
    // AuthModal already stores the token
    setAuthOpen(false);
    if (authRedirect) {
  ...
  return (
    <div style={styles.body}>
          ...
          <div style={styles.heroContent}>
            ...
            <div className="hero-enter hero-enter--delay-4" style={styles.heroButtons}>
              <button 
                ...
                style={{ ...styles.btn, backgroundColor: '#ffffff', color: '#1E4D4B', border: 'none', boxShadow: '0 4px 14px rgba(255, 255, 255, 0.15)', display: 'inline-flex', alignItems: 'center', gap: 8, padding: '14px 28px' }}
              >
              ...
              <button 
                ...
                style={{ ...styles.btn, backgroundColor: 'transparent', color: '#ffffff', border: '2px solid rgba(255, 255, 255, 0.4)', display: 'inline-flex', alignItems: 'center', gap: 8, padding: '14px 28px' }}
              >
      ...
      <section className="cta-pulse" style={styles.ctaBanner}>
        ...
        <button 
          type="button" 
          onClick={() => openAuthModal('signup', '/user-dashboard')} 
          className="reveal" 
          style={{ ...styles.btn, background: 'white', color: '#E76F51', borderColor: 'white', fontSize: 18, padding: '16px 36px', position: 'relative', zIndex: 1 }}
        >
      ...
      {/* Render AuthModal with login success handler */}
      <AuthModal 
        isOpen={authOpen} 
        onClose={() => {
          setAuthOpen(false);
